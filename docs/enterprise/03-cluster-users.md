...
```go
// core/control_plane/defaults.go
...
### SSO Token Validation (Tenant Side)

```go
// core/tenant_node/sso_auth.go

func (tn *TenantNode) ValidateSSOToken(tenantID, token string) (*SSOClaims, error) {
    // Parse JWT
    parsed, err := jwt.Parse(token, func(t *jwt.Token) (interface{}, error) {
        return []byte(tn.config.ControlPlaneJWTSecret), nil
    })
    ...
    // Check admin_access
    if !claims["admin_access"].(bool) {
        return nil, errors.New("no admin access")
    }

    return &SSOClaims{
        UserID:      claims["user_id"].(string),
        TenantID:    claims["tenant_id"].(string),
        AdminAccess: claims["admin_access"].(bool),
    }, nil
}
```
...
**Endpoint**: `GET /api/metrics/tenants/{id}`

```go
func (api *ClusterUserAPI) GetTenantMetrics(c echo.Context) error {
    user := getUserFromContext(c)
    tenantID := c.Param("id")
    ...
    // Get live metrics from node
    metrics := api.cp.GetTenantMetrics(tenantID)
...
<div class="dashboard">
  <header>
    <h1>My Platform</h1>
    <user-menu email="{{ user.email }}"></user-menu>
  </header>
  ...
  <main>
    ...
    <section class="tenants">
      <header>
        <h2>Your Tenants</h2>
        <quota-indicator current="{{ user.quotas.current_tenants }}"
                         max="{{ user.quotas.max_tenants }}"></quota-indicator>
    ...
    <section class="quotas">
      ...
      <quota-item
        label="Tenants"
        :current="user.quotas.current_tenants"
        :max="user.quotas.max_tenants"
        @request-increase="requestQuotaIncrease('max_tenants')"
      ></quota-item>
      <quota-item
        label="Storage per tenant"
        :value="formatBytes(user.quotas.default_storage_per_tenant)"
        @request-increase="requestQuotaIncrease('tenant_storage')"
      ></quota-item>
    </section>
  </main>
</div>
...
See [04-cluster-admin.md](04-cluster-admin.md) for platform operator dashboard and controls.
