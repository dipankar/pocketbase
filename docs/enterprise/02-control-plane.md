...
```go
// core/control_plane/control_plane.go
...
type NodeResources struct {
    ...
    NetworkInRate  int64   `json:"network_in_rate"`  // bytes/sec
    NetworkOutRate int64   `json:"network_out_rate"`
}

type RouteRecord struct {
    TenantID       string    `json:"tenant_id"`
    NodeID         string    `json:"node_id"`
    Priority       int       `json:"priority"`      // 1 = primary, 0 = old (during migration)
    Created        time.Time `json:"created"`
...
type QuotaRecord struct {
    ...
    MaxAPIRequests   int    `json:"max_api_requests"`   // per hour
    MaxConnections   int    `json:"max_connections"`    // concurrent
    MaxCPUMillicores int    `json:"max_cpu_millicores"` // 1000 = 1 core
...
func (f *RaftFSM) Restore(snapshot io.ReadCloser) error {
    ...
    // Restore to BadgerDB
    return f.badger.Load(bytes.NewReader(data), 1)
}
...
func (s *S3SnapshotStore) List() ([]*raft.SnapshotMeta, error) {
    // List snapshots from S3
    result, err := s.s3Client.ListObjectsV2(context.Background(), &s3.ListObjectsV2Input{
    ...
    for _, obj := range result.Contents {
        meta, err := s.parseSnapshotMeta(obj)
        if err == nil {
...
type AffinityRule struct {
    TenantPattern string  // Regex pattern for tenant IDs
...
func (ps *PlacementService) applyRouteAssignment(route RouteRecord) error {
    cmd := Command{
        Type:    "route.assign",
    ...
    future := ps.raft.Apply(mustMarshal(cmd), 5*time.Second)
...
func (ps *PlacementService) getHealthyNodes() ([]NodeRecord, error) {
    var nodes []NodeRecord

    err := ps.badger.View(func(txn *badger.Txn) error {
...
func (hm *HealthMonitor) markNodeDown(nodeID string) {
    // Update node status via Raft
    cmd := Command{
        Type: "node.update",
    ...
    hm.raft.Apply(mustMarshal(cmd), 5*time.Second)

    // Trigger callback
    if hm.onNodeDown != nil {
        hm.onNodeDown(nodeID)
    }

    // Broadcast to gateways
    // (Will cause gateways to invalidate routing cache)
}
```
...
See [03-tenant-nodes.md](03-tenant-nodes.md) for stateless worker implementation.
